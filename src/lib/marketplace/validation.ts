...
export function parseBoundedPagination(searchParams: URLSearchParams): { page: number; pageSize: number } {
  ...
  // Ensure the offset (records skipped) remains a safe integer to prevent overflow/performance issues.
  const offset = (page - 1) * pageSize;
  if (!Number.isSafeInteger(offset)) {
    throw new ValidationError('Pagination offset is too large.');
  }
...
export function validateListingId(raw: string): string {
  ...
  if (!LISTING_ID_PATTERN.test(id)) {
    throw new ValidationError('Listing ID contains invalid characters. Only letters, digits, hyphens, and underscores are allowed.');
  }
  return id;
}
